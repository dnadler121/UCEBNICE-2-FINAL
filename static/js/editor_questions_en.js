(function(){
  const state = {questions_en: window.initialQuestionsEn?.questions_en || []};
  const types = {questions_en:'choice'};
  let fileCounter = 0;
  ...
  function sync(){ const el=document.getElementById('questions_json_en'); if(el) el.value = JSON.stringify(state.questions_en.map(q=>{ const x={...q}; delete x.preview; delete x.previews; return x; })); }
  function newField(){ fileCounter += 1; return `qimg_en_${Date.now()}_${fileCounter}`; }
  ...
  function setupJsonImport(){
    const input=document.getElementById('questionsJsonFileEn');
    const status=document.getElementById('questionsJsonStatusEn');
    const box=document.querySelector('.question-factory-en[data-target="questions_en"]');
    if(!input || !box) return;
    input.addEventListener('change', async()=>{
      ...
      try{
        ...
        state.questions_en.push(...imported);
        renderList(box);
        sync();
  ...
  function renderDynamic(box){
    const target=box.dataset.target || 'questions_en';
  ...
  function moveInputsToBucket(container){
    const bucket = document.getElementById('questionFileBucketEn');
  ...
  function renderList(box){
    const target=box.dataset.target || 'questions_en'; const list=box.querySelector('.question-list'); list.innerHTML='';
  ...
  function addQuestion(box){
    const target=box.dataset.target || 'questions_en'; const question=box.querySelector('.factory-question').value.trim();
  ...
  document.querySelectorAll('.question-factory-en').forEach(box=>{
